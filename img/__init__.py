...
class FileWindow:
    ...
    def len(self):
        return os.path.getsize(self.filename) - self.base_offset if self.wlen is None else self.wlen
"""
A generic resource
"""
    ...
    def parse_body(self, type_table_off, data_off, cnt, recursive=True):
        ...
        for i in range(cnt):
            ...
            if etyp == 's':
                ...
                val = self.parse_body(data_off + val_off + 0x2, data_off, icnt, recursive)
            else:
    ...
    def read_arr(self, k, data_off, str_table, recursive=True):
        ...
        if atyp == 'i':
            ...
            for aval_off in aval_table:
                ...
                aval = self.parse_body(data_off + aval_off + 0x2, data_off, icnt, recursive)
                ret.append(aval)
        else:
...
class Package(Resource):
    ...
    def parse(self, recursive=True):
        ...
        typ, cnt, ptr_off, str_table_off, dec_data_off, dec_len, cmp_len, pad_len = Package.parse_header(data)
        ...
        self.str_table.update(self.fw.read(ptr_off - str_table_off), struct.unpack('=%dI' % cnt, self.fw.read(cnt * 4)))
        self.fw.seek(off)

        for i in range(cnt):
            typ, dec_len, dec_off, flags, is_cmp, cmp_len, cmp_off = Package.parse_entry(self.fw.read(Package.ENTRY_SIZE))
            ...
            if typ in ['TEXI', 'YAML', 'MDL ']:
                elem = SERI(typ, self.str_table.get_str_slot(i), flags, is_cmp, fw, self.str_table)
            elif typ in ['    ']:
...
#            self.str_table.push_str_slot(elem.fn)
        ...
        curr_off = data_off = self.NEXT_BLOCK_ADDR(ptr_off + 0x4 * len(self.str_table.slots))
    ...
    def parse_header(data):
        typ, cnt, ptr_off, str_table_off, dec_data_off, dec_len, cmp_len, pad_len = struct.unpack('=6sH6I', data)
        return typ, cnt, ptr_off, str_table_off, dec_data_off, dec_len, cmp_len, pad_len

    @staticmethod
    def parse_entry(data):
        typ, dec_len, dec_off, flags, is_cmp, cmp_len, cmp_off = struct.unpack('=4s4x6I', data)
    ...
    def get_header(self):
        return 'PAK ', self.dec_len, self.dec_data_off, self.unk, 0x30 if self.typ0 else 0x20
...
class Image(object):
    ...
    def parse(self, recursive=True):
        ...
        for idx in range(idx_table_entry_count):
            typ, num1, num2, num3, num4 = Image.parse_idx_entry(self.fh.read(self.IDX_TABLE_ENTRY_SIZE))
        ...
        for i in range(off_table_entry_count):
            idx, blk_num = Image.parse_off_entry(self.fh.read(self.OFF_TABLE_ENTRY_SIZE))
        ...
        for idx,idx_entry in enumerate(idx_table):
            ...
            if idx_entry[0x0] == 'PAK ':
                self.fh.seek(addr)
                typ, cnt, ptr_off, str_table_off, dec_data_off, dec_len, cmp_len, pad_len = Package.parse_header(self.fh.read(Package.ENTRY_SIZE))
                res = Package(FileWindow(self.filename, addr, cmp_len), idx_entry[0x3])
            else:
                res = Resource(idx_entry[0x0], FileWindow(self.filename, addr, idx_entry[0x1]))
    ...
    def write(self, fh):
        ...
        next_empty_addr = self.NEXT_BLOCK_ADDR(fh.tell() + struct.calcsize('=2I') * off_table_entry_count)
        ...
        # FIXME: All these constants are unknown
        fh.seek(0x0, 0)
        fh.write(struct.pack('=10I',
            0xa,
            data_start_block,
            0x1,
            0x0,
    ...
    def NEXT_BLOCK_ADDR(self, x):
        return x if (x & self.BLOCK_MASK) == 0 else (x & ~self.BLOCK_MASK) + self.BLOCK_SIZE
